...
[[nodiscard]] bool MakePolygon::c(const Device& device)noexcept 
{
    {
    auto vertexBuffersize = sizeof(S);
   ...
   memcpy_s(data, vertexBuffersize, S, vertexBuffersize);

    vertexBuffer_->Unmap(0, nullptr);

    Cvertex_.BufferLocation = vertexBuffer_->GetGPUVirtualAddress();
    Cvertex_.SizeInBytes = vertexBuffersize;
    Cvertex_.StrideInBytes = sizeof(Vertex);
    }
    ...
    Cindex_.BufferLocation = indexBuffer_->GetGPUVirtualAddress();
    Cindex_.SizeInBytes = indexBuffersize;
    Cindex_.Format = DXGI_FORMAT_R16_UINT;
    }
