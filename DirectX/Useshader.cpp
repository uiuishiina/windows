...
{
	const std::string filePath = "shader/PixelShader.hlsl";
	const std::wstring temp = std::wstring(filePath.begin(), filePath.end());

	ID3DBlob* error{};

	auto Compile = D3DCompileFromFile(temp.data(), nullptr, nullptr, "vs", "vs_5_0", D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, 0, &vertexShader_, &error);
	if (FAILED(Compile)) {
		//char* p = static_cast<char*>(error->GetBufferPointer());
		assert(false && "頂点シェーダーのコンパイルに失敗");
	}
	Compile = D3DCompileFromFile(temp.data(), nullptr, nullptr, "ps", "ps_5_0", D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, 0, &pixelShader_, &error);
	if (FAILED(Compile)) {
		char* p = static_cast<char*>(error->GetBufferPointer());
